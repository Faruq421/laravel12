...
export default function FormPage({ auth, item }: PageProps<{ item?: Review }>) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        },
        {
            ...
            href: item ? route('reviews.edit', item.id) : route('reviews.create'),
        },
    ];
    ...
    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        if (item) {
            put(route('reviews.update', item.id));
        } else {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={(item ? 'Edit' : 'Create') + ' Review'} />

            <div className="p-4 sm:p-6 lg:p-8">
                <form onSubmit={handleSubmit}>
                    <Card>
                        <CardHeader>
                            <CardTitle>{item ? 'Edit' : 'Create'} Review</CardTitle>
                        </CardHeader>
                        ...
                        <CardFooter className="flex justify-end gap-2">
                            <Link href={route('reviews.index')} className={cn(buttonVariants({ variant: 'ghost' }))}>Cancel</Link>
