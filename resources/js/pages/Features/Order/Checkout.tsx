    ...
    const handleNextStep = () => {
        if (currentStep === 0) {
            ...
            if (!name || !address || !city || !postal_code || !phone) {
                // Ganti alert dengan mekanisme error yang lebih baik jika perlu
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <Input id="name" type="text" value={data.shipping_address.name} onChange={e => setData('shipping_address', { ...data.shipping_address, name: e.target.value })} required />
                                    {errors['shipping_address.name'] && <p className="text-red-500 text-xs mt-1">{errors['shipping_address.name']}</p>}
                                </div>
                                <div>
                                    <Label htmlFor="phone">Nomor Telepon</Label>
                                    <Input id="phone" type="tel" value={data.shipping_address.phone} onChange={e => setData('shipping_address', { ...data.shipping_address, phone: e.target.value })} required />
                                    {errors['shipping_address.phone'] && <p className="text-red-500 text-xs mt-1">{errors['shipping_address.phone']}</p>}
                                </div>
                            </div>
                            <div>
                                <Label htmlFor="address">Alamat Lengkap</Label>
                                <Input id="address" type="text" value={data.shipping_address.address} onChange={e => setData('shipping_address', { ...data.shipping_address, address: e.target.value })} required />
                                {errors['shipping_address.address'] && <p className="text-red-500 text-xs mt-1">{errors['shipping_address.address']}</p>}
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <Label htmlFor="city">Kota</Label>
                                    <Input id="city" type="text" value={data.shipping_address.city} onChange={e => setData('shipping_address', { ...data.shipping_address, city: e.target.value })} required />
                                    {errors['shipping_address.city'] && <p className="text-red-500 text-xs mt-1">{errors['shipping_address.city']}</p>}
                                </div>
                                <div>
                                    <Label htmlFor="postal_code">Kode Pos</Label>
                                    <Input id="postal_code" type="text" value={data.shipping_address.postal_code} onChange={e => setData('shipping_address', { ...data.shipping_address, postal_code: e.target.value })} required />
